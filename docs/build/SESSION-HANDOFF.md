...
2. **Charter-trim track — evidence gathering is DONE; the decision is at the maintainer's desk** (one-charter rule: trim the ONE charter or nothing; see memory `albion-design-philosophy`). Logs 019/020/021: at n=6/arm headless, real-board engagement was full 1/6 vs lean 4/6 (evidence-complete identical; all 12 solved; Fisher one-tailed p≈0.14 — direction, not significance), and the interactive lean run reproduced everything the full charter has ever demonstrated, including compaction recovery cited line-by-line from the board. The 138-line lean composition (`docs/build/experiments/lean-charter-v1/`) is the candidate trim. NOT yet tested under lean: delegation-heavy work, multi-session continuity, investigative-tier tasks — weigh before adopting. Options for the maintainer: adopt (probably re-adding delegation/investigation sections in trimmed form), gather more n, or keep full and accept headless board unreliability.
