...
**Milestone complete:** **M6 — substantially complete** (build log 015): `install.sh` fresh-machine installer, hook-registration smoke-check (`tests/tools/verify-registration.sh`, live-PASS — closes the inert-hooks class), effort-default fix (doctor `effort` check), CI SHA-pinned + least-privilege, community files + CHANGELOG. Remaining M6 step = plugin-marketplace submission (externally gated, needs maintainer account). Suite = 25 files green.
