...
**Date of handoff:** 2026-07-06 (field-tuning round sealed). **Built, RELEASED (v0.2.0 tagged + marketplace published), and now FIELD-TUNED:** an eight-phase real-world head-to-head (the maintainer's observations record, private) drove the ALB-031…035 round — stop-gate deliverables rule, three verification-fidelity fixtures, two new skills, **charter v0.3** (246 lines, sealed on a pre-registered 16-run A/B: 8/8 evidence-complete boards vs v0.2's 6/8 with revenue-pipeline 0/2), and the session-hygiene orchestration doc. Build log 023. Everything on `main`, 28 test files green. **The packaged plugin + marketplace still carry v0.2.0 — a 0.3.0 release/refresh is the pending outward step (maintainer's call).**
...
- **Charter** `charter/ALBION.md` — **v0.3**, 246 lines (v0.2 trim + ALB-033 verification standards); compiled from `manifest/sections/` by `bin/albion-compile` (`--check` drift gate). Every charter change is sealed behind a pre-registered headless A/B (logs 022, 023).
- **Skills** (`plugin/skills/`): maturity-assessment, delegation, recovery, completion-gate, conductor, hot-path, toolchain-fidelity, + vendored standalone fable-mode. **Agents** (`plugin/agents/`): scout, counterexample-hunter, verifier, simplifier, quick.
...
- **Docs:** README is **purely user-facing** (product page: install, quickstart, coexistence, honest expectations, disclosures); the development history (status, methodology, journal highlights, milestone trail, findings) lives at `docs/build/README.md` — keep diary-type material THERE, not in the README. Plus `docs/glm-5.2-setup.md`, `docs/packaging.md`, `docs/security-model.md`, `docs/build/orchestration.md`, build logs 000–023.
...
### What to do next
1. **Cut release 0.3.0 + refresh the marketplace** (maintainer decision pending): version bump, changelog roll, `bin/albion-package`, update `albion-marketplace/plugins/albion/` + manifest version. The marketplace round-trip verification (2-min, instructions delivered 2026-07-05) can ride the same session.
2. **Watch v0.3 in the field:** the gate proved process non-regression; whether first submissions now match reworked ones on genuinely hard work is only answerable in the next real-world run. The maintainer's observations doc convention (evidence → implication → tuning candidate) is the feed.
3. **Bench follow-ups:** escalation-forcing fixture (nothing yet forces the investigative tier); `board_real` as a first-class metric (hand-computed since log 021); trace field vision availability (doctor's live probe passes here; the marathon ran with placeholders).
4. **M7 — hardening:** provider abstraction (data-sovereignty toggle), interactive conductor steering, lessons promotion.
