...
1. **The permissions deny floor (the hard floor).** Claude Code's permission system enforces deny rules in the client, independent of any prompt or model. This is the real security boundary for the patterns it lists (`rm -rf /`, `sudo rm`, `mkfs`, `dd of=/dev/*`, `curl | sh`, `chmod 777 /`, force-push to protected refs; plus Read/Write denies for `.env`, `**/secrets/**`, `*.pem`, `*.key`, `**/*credentials*`). It is **wired into every `bin/albion` session** via the injected `--settings` layer (`config/albion-settings.json`); stock-Claude users can merge the identical fragment `plugin/settings/permissions-deny.json` manually, and a suite test keeps the two copies from drifting. *(Audit note: before 2026-07-05 this floor was documented but not delivered by the launcher — found and fixed in the post-M6 audit.)*
