...
assert_contains "$RUN_RECORD" "argv[0]=--disable-slash-commands" "vanilla is a true-bare control (no auto-loaded skills)"
assert_contains "$RUN_RECORD" "argv[1]=--model" "vanilla injects model flag"
assert_contains "$RUN_RECORD" "argv[2]=glm-5.2[1m]" "vanilla injects default model"
assert_contains "$RUN_RECORD" "argv[3]=control arm" "vanilla preserves passthrough"
